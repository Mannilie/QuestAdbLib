    ...
    void start(int intervalSeconds) {
        ...
        thread_ = std::thread([this, intervalSeconds]() {
            while (running_) {
                ...
                cv_.wait_for(lock, std::chrono::seconds(intervalSeconds), [this] { return !running_; });
            }
        });
    }
...
QuestAdbManager::~QuestAdbManager() {
...
Result<bool> QuestAdbManager::refreshDeviceList() {
    return getConnectedDevices().success ? Result<bool>::Success(true) : Result<bool>::Error("Failed to refresh device list");
}
    ...
    for (const auto& deviceInfo : devicesResult.value) {
        ...
        if (device.success) {
            ...
            if (!rebootResult.success) {
                allSuccess = false;
                std::cerr << "Failed to reboot device " << deviceInfo.deviceId << ": " << rebootResult.error << std::endl;
            } else {
                auto waitResult = device.value->waitForDevice(defaultConfig_.bootTimeoutSeconds);
                if (!waitResult.success) {
                    allSuccess = false;
                    std::cerr << "Device " << deviceInfo.deviceId << " failed to come back online" << std::endl;
                }
            }
        } else {
            allSuccess = false;
    ...
    for (const auto& deviceInfo : devicesResult.value) {
        ...
        if (device.success) {
            ...
            if (!configResult.success) {
                allSuccess = false;
                std::cerr << "Failed to apply configuration to device " << deviceInfo.deviceId << ": " << configResult.error << std::endl;
            }
        } else {
            allSuccess = false;
        }
...
Result<std::map<std::string, bool>> QuestAdbManager::runCommandOnAll(const std::string& command) {
    auto devicesResult = getConnectedDevices();
    if (!devicesResult.success) {
...
Result<std::map<std::string, std::string>> QuestAdbManager::pullMetricsAll(const std::string& localDirectory) {
...
const HeadsetConfig& QuestAdbManager::getDefaultConfiguration() const {
...
std::string QuestAdbManager::getVersion() {
    return "1.0.0";
}

std::string QuestAdbManager::getBuildInfo() {
    return "QuestAdbLib v1.0.0 - Built with CMake";
}
...
void QuestAdbManager::emitDeviceStatusChange(const std::string& deviceId, const std::string& status) {
