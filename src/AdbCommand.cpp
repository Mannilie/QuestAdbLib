...
namespace QuestAdbLib
{
    ...
    AdbCommand::AdbCommand(const std::string& adbPath)
        : adbPath_(adbPath.empty() ? findAdbPath() : adbPath), pImpl_(std::make_unique<Impl>(adbPath_))
    {
    }
    ...
    std::vector<std::string> AdbCommand::getAdbSearchPaths() const
    {
        ...
        {
            paths.push_back(Utils::joinPath(userProfile, "AppData\\Local\\Android\\Sdk\\platform-tools"));
        }
#else
    ...
    Result<std::string> AdbCommand::run(const std::string& command, const CommandOptions& options)
    {
        ...
        auto result = Utils::executeCommand(fullCommand, options.timeoutSeconds, options.progressCallback);
        ...
        if (options.captureOutput)
        {
            ...
            if (!result.error.empty() &&
                result.error.find("daemon") == std::string::npos &&
    ...
    Result<std::string> AdbCommand::runWithProgress(const std::string& command, ProgressCallback progressCallback)
    {
    ...
    Result<std::vector<DeviceInfo>> AdbCommand::getDevicesWithStatus()
    {
        ...
        for (const auto& line : lines)
        {
            if ((line.find("device") != std::string::npos || line.find("unauthorized") != std::string::npos) &&
    ...
    Result<std::string> AdbCommand::shell(const std::string& deviceId, const std::string& command, bool capture)
    {
        CommandOptions options;
    ...
    Result<std::vector<std::string>> AdbCommand::getRunningProcesses(const std::string& deviceId)
    {
        ...
        for (const auto& line : lines)
        {
            ...
            else if (line.find("PERS") != std::string::npos && line.find(":") != std::string::npos)
            {
                std::regex packageRegex(R"(([a-zA-Z0-9_.]+\.[a-zA-Z0-9_.]+))");
                std::smatch match;
                if (std::regex_search(line, match, packageRegex))
