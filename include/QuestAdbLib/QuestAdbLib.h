...
namespace QuestAdbLib {
    ...
    using DeviceStatusCallback = std::function<void(const std::string& deviceId, const std::string& status)>;
    using DeviceListCallback = std::function<void(const std::vector<DeviceInfo>&)>;
    using MetricsProgressCallback = std::function<void(const std::string& deviceId, double progress)>;
    ...
    public:
        ...
        // Metrics operations
        Result<bool> startMetricsRecordingAll(std::chrono::seconds duration = std::chrono::seconds(30));
        Result<bool> stopMetricsRecordingAll();
        Result<std::map<std::string, std::string>> pullMetricsAll(const std::string& localDirectory);
